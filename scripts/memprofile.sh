...
#
# Requires: a lanternd running with --pprof-addr set and the endpoint reachable
# (default 127.0.0.1:6060; start the daemon with --pprof-addr 127.0.0.1:6060 to
# match this default exactly), plus the `lantern` CLI, `curl`, `jq`, `ps`, and
# `pgrep` on PATH. Runs as an unprivileged user: process RSS and the loopback
# pprof endpoint are both readable without root.
#
# To mirror the mobile builds' buffer sizes, build lanternd with
...
main() {
  ...
  if [ "$DO_MEM" -eq 1 ]; then
    ...
    echo "  go tool pprof -base heap.baseline_connected.pprof  $OUTDIR/heap.idle_end.pprof     # growth over idle soak == slow climb"
    ...
    echo "  go tool pprof -base goroutine.baseline_connected.pprof $OUTDIR/goroutine.idle_end.pprof  # goroutine growth while idle"
    echo "  go tool pprof -base goroutine.baseline_connected.pprof $OUTDIR/goroutine.drain.pprof     # leaked goroutines"
    echo "  (for a slow field-style climb, re-run with a longer soak: IDLE_DURATION=3600)"
  fi
  if [ "$DO_CPU" -eq 1 ]; then
