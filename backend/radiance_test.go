...
func TestUpdateServersReplacesSameTagLanternServer(t *testing.T) {
	dataDir := t.TempDir()
	r := newServerUpdateBackend(t, dataDir)
	historyTime := time.Unix(10, 0).UTC()
	history := &servers.SelectionHistory{
		LastSuccessDelayMs: 120,
		UpdatedAt:          historyTime,
	}
	require.NoError(t, r.srvManager.AddServers(servers.ServerList{Servers: []*servers.Server{{
		Tag:              "same",
		Type:             "direct",
		IsLantern:        true,
		Options:          testDirectOutbound("same", "1.1.1.1"),
		SelectionHistory: history,
	}}}, false))

	err := r.updateServers(servers.ServerList{Servers: []*servers.Server{{
		Tag:       "same",
		Type:      "direct",
		IsLantern: true,
		Options:   testDirectOutbound("same", "2.2.2.2"),
	}}})

	require.NoError(t, err)
	got, found := r.srvManager.GetServerByTag("same")
	require.True(t, found)
	assert.Equal(t, "direct", got.Type)
	out, ok := got.Options.(option.Outbound)
	require.True(t, ok)
	opts, ok := out.Options.(*option.DirectOutboundOptions)
	require.True(t, ok)
	assert.Equal(t, "2.2.2.2", opts.OverrideAddress)
	require.NotNil(t, got.SelectionHistory)
	assert.Equal(t, history.LastSuccessDelayMs, got.SelectionHistory.LastSuccessDelayMs)
	assert.Equal(t, historyTime, got.SelectionHistory.UpdatedAt)
}

func TestUpdateServersDoesNotReplaceSameTagUserServer(t *testing.T) {
	dataDir := t.TempDir()
	r := newServerUpdateBackend(t, dataDir)
	require.NoError(t, r.srvManager.AddServers(servers.ServerList{Servers: []*servers.Server{{
		Tag:       "same",
	...
	got, found := r.srvManager.GetServerByTag("same")
	require.True(t, found)
	assert.False(t, got.IsLantern)
	out, ok := got.Options.(option.Outbound)
	require.True(t, ok)
	opts, ok := out.Options.(*option.DirectOutboundOptions)
	require.True(t, ok)
	assert.Equal(t, "1.1.1.1", opts.OverrideAddress)
}

func TestUpdateServersDoesNotRetainHardDemotionForSameTagReoffer(t *testing.T) {
	dataDir := t.TempDir()
	r := newServerUpdateBackend(t, dataDir)
	require.NoError(t, r.srvManager.AddServers(servers.ServerList{Servers: []*servers.Server{{
		Tag:       "same",
		...
		IsLantern: true,
		Options:   testDirectOutbound("same", "1.1.1.1"),
		SelectionHistory: &servers.SelectionHistory{
			HardDemoted: true,
	...
	require.True(t, ok)
	assert.Equal(t, "2.2.2.2", opts.OverrideAddress)
	if got.SelectionHistory != nil {
		assert.False(t, got.SelectionHistory.HardDemoted)
	}
}

func newServerUpdateBackend(t *testing.T, dataDir string) *LocalBackend {
	t.Helper()
	srvMgr, err := servers.NewManager(dataDir, log.NoOpLogger())
	require.NoError(t, err)
	return &LocalBackend{
...
func testDirectOutbound(tag, overrideAddress string) option.Outbound {
