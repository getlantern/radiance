...
// LocationInfo represents a server location with optional active server details.
// When Active is true, the location has routes in the current config and Tag/Protocol
// identify the specific outbound.
type LocationInfo struct {
	lcommon.ServerLocation
	Tag      string `json:"tag,omitempty"`
	Protocol string `json:"protocol,omitempty"`
	Active   bool   `json:"active"`
}

// AllLocations returns a unified list of all server locations, merging available
// locations from the config response with active outbound details from the server
// manager. Each location indicates whether it has active routes and, if so, the
// protocol in use. This is the single source of truth for the client's location picker.
func (r *Radiance) AllLocations() ([]LocationInfo, error) {
	cfg, err := r.confHandler.GetConfig()
	if err != nil {
		return nil, fmt.Errorf("failed to get config: %w", err)
	}
	...
	// Build a map of active locations from the server manager (outbound tag → location + protocol)
	srvs := r.svrManager.Servers()
	activeByCity := make(map[string]LocationInfo) // key: "city|countryCode"
	if lanternOpts, ok := srvs[servers.SGLantern]; ok {
		for tag, loc := range lanternOpts.Locations {
			key := loc.City + "|" + loc.CountryCode
			// Find the protocol from the outbound type
			protocol := ""
			for _, out := range lanternOpts.Outbounds {
				if out.Tag == tag {
					protocol = out.Type
					break
				}
			}
			activeByCity[key] = LocationInfo{
				ServerLocation: loc,
				Tag:            tag,
				Protocol:       protocol,
				Active:         true,
	...
	// Start with all available locations from the config response
	var result []LocationInfo
	seen := make(map[string]bool)
	for _, loc := range cfg.ConfigResponse.Servers {
		key := loc.City + "|" + loc.CountryCode
		seen[key] = true
		if active, ok := activeByCity[key]; ok {
	...
	// Include any active locations not in the config response (shouldn't happen, but be safe)
	for key, active := range activeByCity {
		if !seen[key] {
			result = append(result, active)
		}
	}
