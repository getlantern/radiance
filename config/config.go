...
const configFileName = "client.conf"
...
func NewConfigHandler(configDir string, pollInterval time.Duration) *ConfigHandler {
	ch := &ConfigHandler{
...
func (ch *ConfigHandler) fetchConfig() error {
	...
	if resp != nil {
		...
		if proxyList != nil && len(proxyList.GetProxies()) > 0 {
			...
			if sErr := saveConfig(ch.configPath, proxies); sErr != nil {
...
func (ch *ConfigHandler) loadConfig() error {
	...
	ch.config.Set(configResult{cfg: cfg})
	return nil
}
