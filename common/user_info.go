...
func (u *userInfo) SetData(data *common.UserData) error {
	// We get user data from several different calls to the API, some of which
	// may not have all the fields populated. Therefore, we only update the fields
	// that are non-empty in the new data.
	if u.data != nil {
		if data.IP == "" {
			data.IP = u.data.IP
		}
		if data.Country == "" {
			data.Country = u.data.Country
		}
	}
...
func save(data *common.UserData, path string) error {
	...
	if data == nil {
		// Remove the file if data is nil
