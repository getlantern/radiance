...
func InitIPC(settingsFilePath string, provider func() libbox.PlatformInterface) (*ipc.Server, error) {
	if ipcServer != nil {
		// already started
		return ipcServer, nil
	}
	platIfceProvider = provider
	if err := common.InitReadOnly(settingsFilePath); err != nil {
	...
	return ipcServer, ipcServer.Start(settingsFilePath, func(ctx context.Context, group, tag string) (ipc.Service, error) {
