	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if len(tt.lanternTags) > 0 {
				boxOptions.Options = cfg.Options
			}
			if len(tt.userTags) > 0 {
				boxOptions.UserServers = O.Options{
					Outbounds: svrs[servers.SGUser].Outbounds,
					Endpoints: svrs[servers.SGUser].Endpoints,
				}
			}
			...
			hasGroupWithTags(t, gotOutbounds, AutoSelectTag, []string{AutoLanternTag, AutoUserTag})

			assert.FileExists(t, filepath.Join(path, internal.DebugBoxOptionsFileName), "debug option file must be written")
		})
	}
}
