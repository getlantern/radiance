...
nameserver ` + serverHost + "\n")
	...
	log.Debugf("DNS server set to %s", serverHost)
	return nil
}
...
func restoreSystemDNSServer() {
	for _, backup := range systemDNSBackups {
		if _, err := os.Stat(backup.backup); err == nil {
			...
			log.Debugf("DNS config restored from '%s' to '%s'\n", backup.backup, backup.original)
		} else if errors.Is(err, os.ErrNotExist) {
			// backup not exist - just remove original, because it's created by ourselves
			if err := os.Remove(backup.original); err != nil {
				log.Errorf("failed to remove Outline DNS config file '%s': %v\n", backup.original, err)
				continue
			}
			log.Debugf("Outline DNS config '%s' has been removed\n", backup.original)
		} else {
			log.Debugf("failed to check the existence of DNS config backup file '%s': %v\n", backup.backup, err)
		}
	}
}
